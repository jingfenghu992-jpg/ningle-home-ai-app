...
3. **絕對唔可以** 亂作資料。`;
    ...
    let visionContext = "";
    if (visionSummary) {
        visionContext = `\n\n【重要：視覺分析資料】\n用戶剛上傳了圖片，以下是 AI 視覺分析報告，請必須引用此內容回答用戶問題（例如「見到你張相...」）：\n${visionSummary}\n\n如果是顧問模式（Consultant Mode），請你遵循以下「手機版簡潔回覆」原則：\n1. **結構確認**：用一句話確認空間結構特徵（例如橫樑、窗台）。\n2. **單一精準建議**：針對該結構，只提供 **1 個** 最實用嘅訂造建議（切勿一次過俾太多）。\n3. **風格建議**：簡短建議一種風格。\n4. **互動引導**：詢問客人「覺得點樣？定係想聽下其他方案？」。\n5. **視覺引導**：最後必須禮貌提到：「想睇實際效果，可以點擊上方嘅【智能設計】，即刻出圖俾你睇！✨」\n`;
    }
...
5. **多樣化建議**：如果客人問及其他方案，請提供與之前不同但同樣實用嘅建議。`;
    }
