...
**Production URL（Vercel）**：`https://ningle-home-ai-dl0gaow58-kings-projects-bb32b161.vercel.app`

---

## 13) 香港上传后体验流程简化（本轮落地情况）

> 范围：只改“卡片内文案/显示逻辑/选项数量/重复渲染 bug/聊天规则”，不改首页与 UI 外观框架。

### 13.1 改动文件

- `App.tsx`
  - 上传后固定先出：`【圖片分析】`（短、繁体、香港口吻；纯描述）
  - `【布置方案 A/B】`：同一轮只出一张；文案短版；按钮仅「方案 A / 方案 B」
  - `【風格選擇】`：风格 3 + 目标 2 + 强度 2（单选）；按钮「一鍵出圖」
  - 出图提示：口语化；移除敏感/技术字眼
  - 出图后：`【設計重點】`（4–6 点短句）；移除所有精修按钮入口；后续只用聊天输入收集改图
  - 改图次数：内部 `revisionIndex` 控制（超过阈值后仅 WhatsApp 引导，不向用户提及次数/限制）
- `api/chat.js`
  - System 规则：只收集改图信息；不回答材料/价钱/施工；每次 2–3 个短问题收敛；需要时转 WhatsApp（不提次数/限制）

### 13.2 验收清单（代码级对照）

- 上传后先见 `【圖片分析】`：✅
- A/B 卡只出现一次：✅
- 选项最少化（单选）：✅（风格 3、目标 2、强度 2）
- 出图提示无技术/敏感字眼：✅
- 出图后只有 `【設計重點】`，无精修按钮：✅
...
### 15.6 Mobile-First（375/390/414）统一（本次）

- **布局/边距**：聊天区与输入条统一 `px-4` gutter + `max-w-[520px]`，手机为默认样式，大屏仅放宽最大宽度不改布局。
- **触控尺寸**：选项全部使用短按钮（chip/pill），默认两列/多列；主操作按钮（如出图）为单颗 `h-11`。
- **图片分析卡**：最多 5 行，每行「标签：内容」短句，行距统一。
- **布置方案卡**：A/B 每个方案最多 4 行要点，避免长文本把按钮挤出屏幕。
- **输入条**：移除外层额外 padding，保证与聊天内容左右对齐；底部保留 safe-area padding。

#### 15.6.1 自测（手机优先）

- **iPhone 12/13（390）**：空间选择两列 chips，按钮可点、不溢出；风格/目标/强度为多列 chips；出图按钮单颗明显但不霸屏。
- **分析卡**：不超过 5 行，字体 14px、行高一致。
- **方案 A/B**：文本不撑爆屏幕，A/B 按钮始终在卡片可视范围内。
- **对齐**：底部输入条与聊天内容左右 gutter 一致。
- **重复卡片**：同一步骤只保留一张卡（使用 upsert 更新替换）。
- **敏感词**：用户界面不出现 AI/模型/图生图/文生图/prompt/debug/风险/禁止/强制/renderId/锁定 等字样。
