...
- `App.tsx`
  - FAST 流程：`【圖片分析】` 短版（繁体、港式口吻、纯描述）
  - `【布置方案 A/B】`：去重（同一 uploadId 只触发一次）+ 文案短版（每方案 5 行）+ 选项改为「方案 A / 方案 B」
  - `【風格選擇】`：风格 3 + 目标 2 + 强度 2（单选）+ 主按钮统一为「一鍵出圖」
  - 出图提示：改口语，移除技术词/禁词
  - 出图后：改为 `【設計重點】`（4–6 点短句）并移除所有精修按钮入口
  - 聊天改图：使用内部 `revisionIndex` 控制（>2 后不再出图，转 WhatsApp；对用户不提任何次数/限制字眼）
- `api/chat.js`
  - System 规则：只收集改图信息（收纳/床/衣柜/书枱/灯光/色温/窗帘/地台/动线），不回答材料/价钱/施工等，收敛式 2–3 个短问题，必要时转 WhatsApp（不提次数/限制）
- `REPORT.md`
  - Step 1 文案来源盘点 + 本节验收记录

### 13.2 验收清单（代码级对照 + 本地构建结果）

- **上传后先见 `【圖片分析】`**：已改为固定 5–6 行短句，且移除“风险/禁止/强制”等禁词（FAST 分析卡）。
- **`【布置方案 A/B】` 只出现一次**：已对 `uploadId` 加去重锁，避免重复触发导致重复卡片。
- **选项最少化（单选）**
  - 风格：3 个（現代簡約／奶油暖白／日式木系）
  - 目标：2 个（收納優先／顯大清爽）
  - 强度：2 个（更貼近原相／設計感多啲）
- **出图提示无技术词/禁词**：前端已移除“图生图/次数/最多/超过”等字眼；出图后引导改为“打几句我再出一张”。
- **出图后只显示 `【設計重點】`，无精修按钮**：已删除旧的 `再精修` 与快捷微调按钮逻辑入口，卡片改为短版重点。
- **聊天改图最多再出 2 次，之后转 WhatsApp（不提次数/限制）**：已用 `revisionIndex`（0/1/2）控制；当再提出修改且达到阈值后仅回复 WhatsApp 引导文案。
- **本地构建**：`npm ci` + `npm run build` ✅ 通过（`tsc && vite build`）。

> 需要你在浏览器用同一张「小睡房毛坯相」按 1→8 顺序实际走一遍（含 2 次改图 + 第 3 次请求改图），确认线上 Vercel Production 行为与文案一致。
