...
export function buildCameraLock(hkAnchorsLite) {
  // <= 160 chars, hard constraints
  const a = hkAnchorsLite || {};
  const view = String(a.camera_view || '').toLowerCase();
  
  let cam = 'Realistic smartphone perspective. Match input photo camera angle.';
  if (view === 'frontal') cam = 'Frontal view (1-point perspective). Match input photo angle.';
  if (view === 'angled') cam = 'Angled view (2-point perspective). Match input photo angle.';

  return `${cam} No wide-angle distortion. Keep vertical lines straight.`;
}
...
export function buildStructureLock(hkAnchorsLite) {
  // Hard constraints for i2i "match original" mode (never dropped).
  // Optional: add extra structure rules derived from hkAnchorsLite (FAST mode).
  const a = hkAnchorsLite && typeof hkAnchorsLite === 'object' ? hkAnchorsLite : null;
  const winWall = String(a?.window_wall || '').trim().toLowerCase();
  const winCount = a?.window_count;
  const beam = String(a?.beam_present || '').trim().toLowerCase();
  const col = String(a?.column_present || '').trim().toLowerCase();
  const lensRisk = String(a?.lens_risk || '').trim().toLowerCase();

  const extra = [];
  if (winWall === 'far' && (winCount === 1 || String(winCount) === '1')) {
    extra.push('Only one window on the far wall. No windows on left/right walls.');
  }
  // Stronger constraint against fisheye unless explicitly risking it
  if (lensRisk !== 'wide_risk') {
    extra.push('No wide-angle/fisheye/vignette. Match input photo POV exactly.');
  } else {
    extra.push('Match input photo POV exactly. Keep vertical lines straight.');
  }
  if (beam === 'none' && col === 'none') {
    extra.push('Do not add beams or columns.');
  }

  return [
    'STRUCTURE LOCK: Keep the original room geometry exactly the same.',
    'Keep window and door positions EXACTLY as in the input photo.',
...
function sizeHintFromIntake(intake) {
  ...
  const d = normalize(intake?.roomDepthChi);
  const h = normalize(intake?.roomHeightChi);

  // Parse "8–10尺" -> "8-10 feet"
  const parseFeet = (str) => {
    const s = String(str || '').replace(/尺/g, 'ft').replace(/[–—]/g, '-');
    return s || '';
  };

  const wFt = parseFeet(w);
  const dFt = parseFeet(d);
  const hFt = parseFeet(h);

  if (wFt || dFt || hFt) {
    const parts = [];
    if (wFt) parts.push(`Width: approx ${wFt}.`);
    if (dFt) parts.push(`Depth: approx ${dFt}.`);
    if (hFt) parts.push(`Height: approx ${hFt}.`);
    parts.push('Realistic Hong Kong apartment scale.');
    return parts.join(' ');
  }

  return 'Compact scale. Realistic Hong Kong apartment proportions.';
}
...
const GLOBAL_LOCK = [
  'Hong Kong apartment interior.',
  'Photorealistic 8k.',
  'Buildable design.',
  'Ceiling height approx 2.6m.',
  'Eye-level view.',
  'Level horizon.',
  'Normal lens.',
  'NO wide-angle.',
...
const NEGATIVE_COMMON = [
  ...
  // Double insurance against openings/lens artifacts
  'new window, side window, extra window, balcony door, wide angle, fisheye, panoramic, vignette, dark corners, circular frame, warped, stretched, lens distortion, curved lines, tunnel view',
].join(' ');
 ...
 */
export function buildHKPrompt({ renderIntake, hkAnchors, hkAnchorsLite, spaceType, layoutVariant, sizeChoice, styleChoice }) {
  ...
  const cameraLock = buildCameraLock(hkAnchorsLite);
  const noVignetteLock = buildNoVignetteLock();
  const inferredLite =
    hkAnchorsLite ||
    intake?.hkAnchorsLite ||
    intake?.visionExtraction?.hkAnchorsLite ||
    intake?.extraction?.hkAnchorsLite ||
    null;
  const structureLock = buildStructureLock(inferredLite);
