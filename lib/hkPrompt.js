...
export function buildHKPrompt({ renderIntake, hkAnchors, spaceType, layoutVariant, sizeChoice, styleChoice }) {
  ...
  const order = ['global', 'anchor', 'space', 'size', 'layout', 'style', 'light', 'negative'];
  ...
  if (prompt.length > 1024) {
    // Final fallback: preserve hard parts (global/anchor/space/size/layout/negative), then slice.
    const keepKeys = ['global', 'anchor', 'space', 'size', 'layout', 'negative'].filter(k => keys.includes(k));
